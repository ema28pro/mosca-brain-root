...
class CompoundEye:
    ...
    def feed_frame(
        ...
        mode: str = "luminance",     # "luminance", "edges", "contrast"
    ) -> Tuple[np.ndarray, np.ndarray, float]:
        """
        Alimenta directamente un fotograma de un juego (pantalla, cámara o simulación)
        a los ojos compuestos de la mosca.

        Args:
            frame: array numpy (Alto, Ancho) en escala de grises o (Alto, Ancho, 3) RGB.
            stereo_mode:
            ...
            mode: 'luminance' (brillo directo), 'contrast' o 'edges'.

        Returns:
            (left_intensities, right_intensities, looming_signal)
        """
        arr = np.asarray(frame, dtype=np.float32)

        # Convertir RGB a escala de grises si es necesario
        if arr.ndim == 3:
            if arr.shape[2] == 3:
                # Conversión estándar RGB a luminancia
        ...
        if stereo_mode == "split":
            # Ojo izquierdo toma la mitad izquierda, Ojo derecho toma la mitad derecha
            mid = w // 2
            left_img = arr[:, :mid]
            right_img = arr[:, mid:]
        else:
            # "duplicate": ambos ojos reciben la imagen completa
            left_img = arr
            right_img = arr

        # Redimensionar / muestrear por cada omatidio promediando bloques verticales
        # Cada omatidio muestrea una columna angular del campo visual
        left_samples = np.zeros(self.n_left, dtype=np.float32)
        step_l = left_img.shape[1] / self.n_left
        for i in range(self.n_left):
            col_start = int(i * step_l)
            col_end = max(col_start + 1, int((i + 1) * step_l))
            left_samples[i] = float(np.mean(left_img[:, col_start:col_end]))

        right_samples = np.zeros(self.n_right, dtype=np.float32)
        step_r = right_img.shape[1] / self.n_right
        for i in range(self.n_right):
            col_start = int(i * step_r)
            col_end = max(col_start + 1, int((i + 1) * step_r))
            right_samples[i] = float(np.mean(right_img[:, col_start:col_end]))

        # Detección de peligro por cambio brusco en oscuridad (sombra repentina en el juego)
        current_mean = float(np.mean(arr))
        looming = 0.0
        if hasattr(self, "_prev_frame_mean"):
            diff = self._prev_frame_mean - current_mean
            if diff > 0.35:  # Si la pantalla se oscurece rápidamente (amenaza inminente)
                looming = float(np.clip(diff * 3.0, 0.0, 2.0))
        self._prev_frame_mean = current_mean

        self.left_intensities = np.clip(left_samples * self.config.sensitivity, 0.0, 1.5)
        self.right_intensities = np.clip(right_samples * self.config.sensitivity, 0.0, 1.5)
        self.looming_signal = looming
